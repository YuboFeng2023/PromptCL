...
def map_evt_to_tokens_for_text(evt, prob=0):
    # evt ='<cls> ' + ' <sep> '.join(evt) + ' <sep>'
    # evt = [evt[1],evt[0],evt[2]]
    tmp = np.random.binomial(n=1, p=prob)
    if tmp:
        evt = ['subject is', evt[0], 'predicate is', evt[1], 'object is', evt[2]]
    evt = ' '.join(evt)
    evt = evt.replace("''", "").lower().split()
    evt = ' '.join([tokenizer.cls_token] + evt + [tokenizer.sep_token])
    return evt
...
def map_evt_to_types_for_text(evt):
    l = ['cls']
    for i, t in enumerate(['s', 'p', 'o']):
        e = evt[i]
        e = e.replace("''", "").lower()
        e = tokenizer.map_text_to_id(e)
        for token in e:
            l.append(t)
    l.append('sep')
    return l
...
class TrainData(tx.data.DatasetBase[Example, Example]):
    ...
    def process(self, raw_example):
        ...
        mask = random.choice(['s', 'p', 'o', 'sp', 'so', 'po', 'spo'])
        mask_pos_list, mask_id_list = list(), list()
        for mask_pos, t in enumerate(evt_q_type):  # selecting
            if t in mask:  # masking
        ...
        evt_k1 = map_evt_to_tokens_for_text(evt_k_str, prob)
        evt_k1_ids = tokenizer.map_text_to_id(evt_k1)
        evt_k2 = map_evt_to_tokens_for_text(evt_k_str, prob)
        evt_k2_ids = tokenizer.map_text_to_id(evt_k2)
        ...
        return {
            ...
            "evt_q_ids": evt_q_ids,
            "evt_k_ids": evt_k_ids,
            "evt_k1_ids": evt_k1_ids,
            "evt_k2_ids": evt_k2_ids,
            "evt_p_ids": evt_p_ids,
            "mask_pos": mask_pos,
    ...
    def collate(self, examples: List[Example]) -> tx.data.Batch:
        ...
        evt_k1_ids, evt_k1_lengths = tx.data.padded_batch(
            [ex["evt_k1_ids"] for ex in examples], pad_value=pad_token_id)
        evt_k2_ids, evt_k2_lengths = tx.data.padded_batch(
            [ex["evt_k2_ids"] for ex in examples], pad_value=pad_token_id)
        ...
        mask_id_list, mask_id_lengths = tx.data.padded_batch(
            [ex["mask_id_list"] for ex in examples], pad_value=pad_token_id)
        mask_pos_list, mask_pos_lengths = tx.data.padded_batch(
            [ex["mask_pos_list"] for ex in examples], pad_value=pad_token_id)
        ...
        return tx.data.Batch(
            ...
            evt_k1_ids=torch.from_numpy(evt_k1_ids),
            evt_k1_lengths=torch.tensor(evt_k1_lengths),
            evt_k2_ids=torch.from_numpy(evt_k2_ids),
            evt_k2_lengths=torch.tensor(evt_k2_lengths),
            ...
            mask_id_list=torch.from_numpy(mask_id_list),
            mask_id_lengths=torch.tensor(mask_id_lengths),
            mask_pos_list=torch.from_numpy(mask_pos_list),
            mask_pos_lengths=torch.tensor(mask_pos_lengths),
        )
